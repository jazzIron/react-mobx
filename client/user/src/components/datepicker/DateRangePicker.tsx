...
import DatePicker, { DateRangePicker as ToastDateRangePicker } from 'tui-date-picker';
import { IToastDTP } from './datepicker_types';

export function DateRangePicker({ useTime, onChange, defaultDate }: IToastDTP) {
  const [tui, setTui] = useState<ToastDateRangePicker>();
  ...
  useEffect(() => {
    if (tui === undefined) {
      ...
      tui.on('change:end', () => (typeof onChange === 'function' ? onChangeHandler() : undefined));
    }
  ...
  return (
    <>
      ...
      <span> - </span>
      <div className="tui-datepicker-input tui-datetime-input tui-has-focus">
        <input id="endpicker-input" ref={tuiEndInputRef} type="text" aria-label="Date" readOnly />
        <span className="tui-ico-date"></span>
        <div id="endpicker-container" ref={utiEndWrapperRef} style={{ marginLeft: '-1px' }}></div>
      </div>
    </>
  );
}
